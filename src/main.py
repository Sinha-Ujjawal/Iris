...
from classifiers import dummy_clf, logistic_clf
...
test_size = st.slider(
    label="Test Splot", min_value=0.1, max_value=0.4, step=0.05, value=0.2
)
...
CLASSIFIERS: Dict[str, BaseEstimator] = {"Dummy": dummy_clf, "Logistic": logistic_clf}

clf_name = st.selectbox(
    label="Choose the classifier model", options=sorted(CLASSIFIERS.keys())
)
...
st.write(f"### Prediction: {prediction}, i.e. {target_names[prediction]}")
